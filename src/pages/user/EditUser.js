    ...
    const formik = useFormik({
        initialValues: {
            ...
            try {
                ...
                let r;
                if (op === "add") {
                    r = await registerUser(values.username, values.password, values.nickname, values.role, values.doubanUser, values.qywxUser, values.overseerrUser, values.pushdeerKey, values.barkUrl,values.scoreRuleName)
                } else {
                    r = await updateUser(id, values.username, values.nickname, values.password, values.role, values.doubanUser, values.qywxUser, values.overseerrUser, values.pushdeerKey, values.barkUrl,values.scoreRuleName)
                }
                if (r.code === 0) {
